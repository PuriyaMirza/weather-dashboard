...
// This page fetches live weather on every request; without this it would be statically
// prerendered once at build/deploy time and serve that frozen snapshot forever.
export const dynamic = 'force-dynamic';

const DEFAULT_PLACE = 'Portland, Oregon';

async function loadWeather(): Promise<{ data?: WeatherDashboardData; errorMessage?: string }> {
  try {
    const data = await getWeatherForPlaceName(DEFAULT_PLACE);
    return { data };
  } catch (error) {
    const message = error instanceof WeatherLookupError ? error.message : 'Unable to load weather data.';
    return { errorMessage: message };
  }
}

export default async function Home() {
  const { data, errorMessage } = await loadWeather();
      ...
      <section className="mx-auto max-w-7xl" aria-labelledby="dashboard-title">
        <div className="rounded-[2rem] border border-white/70 bg-white/75 p-6 shadow-xl shadow-slate-200/80 backdrop-blur sm:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-sky-700">Milestone 4 · Live data</p>
              ...
              <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600 sm:text-lg">
                Responsive, accessible dashboard shell powered by normalized TypeScript weather data and a card registry, sourced live from Open-Meteo. Location search isn&apos;t available yet.
              </p>
            </div>
